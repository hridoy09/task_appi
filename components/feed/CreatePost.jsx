...
import useFeedHandaler from "./_hooks/useFeedHandaler";
...
const PRIVACY_OPTIONS = [
  { value: "public", label: "Public", description: "Anyone", Icon: PublicIcon },
  { value: "friends", label: "Friends", description: "Your friends only", Icon: FriendsIcon },
...
const CreatePost = () => {
  ...
  return (
    ...
    >
      ...
      />
      <div className="_feed_inner_text_area_box position-relative">
        ...
        >
          <button
            type="button"
            className="btn btn-light btn-sm d-flex align-items-center gap-2 rounded-pill border text-secondary"
            onClick={() => setIsPrivacyOpen((value) => !value)}
            aria-expanded={isPrivacyOpen}
          >
            <selectedPrivacy.Icon />
            <span className="fw-medium">{selectedPrivacy.label}</span>
            <ChevronIcon />
          </button>

          {isPrivacyOpen ? (
            <div
              className="position-absolute bg-white border rounded shadow-sm _padd_t8 _padd_b8"
              style={{ top: "calc(100% + 6px)", right: 0, minWidth: 220, zIndex: 20 }}
            >
              {PRIVACY_OPTIONS.map((option) => (
                <button
                  key={option.value}
                  type="button"
                  className={`btn btn-sm w-100 text-start d-flex align-items-center gap-2 rounded-0 ${
                    option.value === privacy ? "bg-light" : ""
                  }`}
                  onClick={() => {
      ...
      {photoPreview ? (
        <div className="position-relative d-inline-block _mar_b14">
          <img
            src={photoPreview}
          ...
          <button
            type="button"
            className="btn btn-sm btn-dark rounded-circle position-absolute d-flex align-items-center justify-content-center"
            style={{ top: 8, right: 8, width: 28, height: 28, padding: 0, opacity: 0.85 }}
            onClick={clearMedia}
      ...
      {type !== "text" ? (
        <div className="d-flex justify-content-end _mar_b14">
          <span className="badge text-bg-light border text-secondary text-capitalize">
            {type}
          </span>
        </div>
      ) : null}
